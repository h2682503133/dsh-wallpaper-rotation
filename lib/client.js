...
window.__ModuleLoader__.load({ id: 'dsh-wallpaper-rotation', factory: (require) => {
  ...
  /** One-liner the Plugins page renders as this row's description. */
  const SUMMARY = '按主题（浅色/深色）分组，定时轮播本地壁纸图片。'

  function SettingsCard(props) {
    // `plugins.row.config` asks for `view: 'summary'` when the Plugins page
    // wants this row's one-liner; the editor belongs to `view: 'page'`.
  ...
  function apply(ctx) {
    ...
    // 0.1.7 removed `settings.plugin.item`. A bundle's configuration belongs in
    // `plugins.row.config`, keyed `<package name>#<row id>` — that is what puts
    // a configure control on this plugin's row in the Plugins page.
    ctx.slots.inject('plugins.row.config', () => ctx.slots.register({
      name: 'plugins.row.config',
      key: 'dsh-wallpaper-rotation#dsh-wallpaper-rotation',
      inject: () => ({}),
    }, SettingsCard))
  }
